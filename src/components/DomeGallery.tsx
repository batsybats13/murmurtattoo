...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  // Если изображений больше чем слотов, некоторые не будут показаны
  const normalizedImages = pool.map((image) => {
    if (typeof image === "string") {
      return { src: image, alt: "", title: "", description: "", allImages: [] };
    }
    return {
      src: image.src || "",
      alt: image.alt || "",
  ...
  return coords.map((c, i) => ({
    ...
    allImages: usedImages[i].allImages,
  }));
}
...
export default function DomeGallery({
  ...
  const openItemFromElement = useCallback(
    (el: HTMLElement) => {
      ...
      // Получаем данные из data-атрибутов, используя getAttribute для надежности
      const itemTitle = parent.getAttribute("data-title") || "";
      const itemDescription = parent.getAttribute("data-description") || "";
      const itemSrc = parent.getAttribute("data-src") || "";
      const itemAlt = el.querySelector("img")?.getAttribute("alt") || "";

      let allImages: string[] = [];
      try {
        const imagesData = parent.getAttribute("data-all-images");
        if (imagesData) {
      ...
      // Если есть onItemClick, используем его вместо стандартного открытия
      if (onItemClick) {
        try {
  ...
  const onTileClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      e.stopPropagation();
      e.preventDefault();
      // Если активно перетаскивание, не обрабатываем клик
      if (draggingRef.current) {
        return;
      }
      // Если только что закончили перетаскивание, не обрабатываем клик
      ...
      <main ref={mainRef} className="sphere-main">
        <div className="stage">
          <div ref={sphereRef} className="sphere">
            {items.map((it, i) => {
              const allImagesJson = JSON.stringify(it.allImages || []);
              return (
                <div
                  key={`${it.x},${it.y},${i}`}
                  className="item"
                ...
                >
                  <div
                    ...
                    onMouseDown={(e) => {
                      // Предотвращаем перетаскивание при клике
                      if (e.button === 0) {
                        e.stopPropagation();
                      }
                    }}
                  >
                    ...
                    <div
                      ...
                      data-length={
                        !it.title && !it.alt
                          ? "medium"
                          : (it.title || it.alt || "").length <= 8
                          ? "short"
                          : (it.title || it.alt || "").length <= 18
                          ? "medium"
                          : "long"
                      }
                    >
                      {it.title || it.alt || "Без названия"}
                    </div>
                  </div>
                </div>
              );
